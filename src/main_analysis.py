...
class IntegratedAnalyzer:
    ...
    def _fuse(self, video_report: dict, audio_report: dict | None) -> dict:
        ...
        self._dispatch_alerts(integrated)
        return integrated

    @staticmethod
    def _dispatch_alerts(data: dict):
        fusion = data["fusao_multimodal"]
        alerts = []
        ts = data["timestamp"]

        dep = fusion["depressao"]
        if dep["risco"] in ("ALTO", "MUITO ALTO"):
            alerts.append({
                "tipo": "DEPRESSÃO",
                "nivel": dep["risco"],
                "score": dep["score_total"],
                "destinatario": "Equipe de Saúde Mental",
                "acao": "Agendar avaliação psicológica urgente",
            })

        vio = fusion["violencia_domestica"]
        if vio["hematomas"] > 0:
            alerts.append({
                "tipo": "VIOLÊNCIA DOMÉSTICA",
                "nivel": vio["risco"],
                "hematomas": vio["hematomas"],
                "destinatario": "Serviço Social + Equipe Médica",
                "acao": "Acionar protocolo de acolhimento e notificação compulsória",
            })

        saude = fusion["saude"]
        if saude["marcas"] > 0:
            alerts.append({
                "tipo": "ANOMALIA DERMATOLÓGICA",
                "nivel": "MODERADO",
                "marcas": saude["marcas"],
                "destinatario": "Equipe Médica (Dermatologia)",
                "acao": "Solicitar avaliação presencial",
            })

        if not alerts:
            log.info("Nenhum alerta disparado — indicadores dentro da normalidade")
            return

        sep = "=" * 72
        alert_path = "alertas_equipe_medica.txt"
        with open(alert_path, "w", encoding="utf-8") as f:
            f.write(f"{sep}\n")
            f.write("⚠️  SISTEMA DE ALERTAS — EQUIPE MÉDICA\n")
            f.write(f"{sep}\n")
            f.write(f"Data/Hora: {ts}\n")
            f.write(f"Paciente (vídeo): {data['arquivo']}\n")
            f.write(f"Total de alertas: {len(alerts)}\n\n")

            for i, a in enumerate(alerts, 1):
                f.write(f"--- ALERTA #{i}: {a['tipo']} ---\n")
                f.write(f"  Nível: {a['nivel']}\n")
                f.write(f"  Destinatário: {a['destinatario']}\n")
                f.write(f"  Ação recomendada: {a['acao']}\n")
                for k, v in a.items():
                    if k not in ("tipo", "nivel", "destinatario", "acao"):
                        f.write(f"  {k}: {v}\n")
                f.write("\n")

            f.write(f"{sep}\n")
            f.write("IMPORTANTE: Este alerta foi gerado automaticamente.\n")
            f.write("A equipe médica deve avaliar pessoalmente a paciente.\n")
            f.write(f"{sep}\n")

        alert_json = "alertas_equipe_medica.json"
        with open(alert_json, "w", encoding="utf-8") as f:
            json.dump({"timestamp": ts, "arquivo": data["arquivo"], "alertas": alerts}, f, indent=2, ensure_ascii=False)

        log.warning("🚨 %d ALERTA(S) DISPARADO(S) para equipe médica → %s", len(alerts), alert_path)
        for a in alerts:
            log.warning("  → [%s] %s — Dest: %s", a["nivel"], a["tipo"], a["destinatario"])
